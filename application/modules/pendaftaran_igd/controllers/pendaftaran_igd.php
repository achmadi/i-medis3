 ...
 * Pendaftaran IGD.
 * 
 * @package Pendaftaran_IGD
 * @category Controller
 * @author Dadang Dana Suryana
 */
class Pendaftaran_IGD extends Admin_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Pendaftaran_IGD_Model');
		...
		$this->Pendaftaran_IGD_Model->delete_expire_no_register_from_queue();
		$this->Pendaftaran_IGD_Model->delete_expire_no_medrec_from_queue();
	}
	...
	private function _update($id = 0) {
        ...
        if ($this->input->post('batal')) {
            redirect('pendaftaran_igd', 'refresh');
        }
        ...
        if ($this->input->post('simpan')) {
            if ($this->form_validation->run() == TRUE) {
                ...
                if (isset($pendaftaran->pendaftaran_id) && $pendaftaran->pendaftaran_id > 0) {
                    $this->Pendaftaran_IGD_Model->update($pendaftaran);
                }
                else {
                    $id = $this->Pendaftaran_IGD_Model->create($pendaftaran);
                }
				$baru = $this->input->post('baru');
				redirect('pendaftaran_igd/index?print='.$id.'&baru='.$baru, 'refresh');
            }
        }
		
		if ($id) {
			$pendaftaran = $this->Pendaftaran_IGD_Model->getBy(array('pendaftaran_igd.id' => $id));
		...
		$this->data['dokter_list'] = $dokters['data'];
		
		$this->data['pj_hubungan_list'] = $this->Pendaftaran_IGD_Model->getHubunganDenganPasien();
		
		$this->data['status_kawin_list'] = status_kawin();
		
		$this->data['top_nav'] = "pendaftaran_igd/top_nav";
		$this->data['top_nav_selected'] = "Pendaftaran";
		$this->data['sub_nav'] = "pendaftaran_igd/sub_nav";
		$this->template->set_title('Pendaftaran IGD')
					   ->set_css('datepicker')
					   ->set_js('bootstrap-datepicker')
					   ->set_js('jquery.dataTables')
			           ->build('pendaftaran_igd/edit');
    }
	...
	public function browse($browse = 0) {
		...
		$this->template->set_title('Pendaftaran IGD')
					   ...
					   ->set_js('alertify.min')
			           ->build('pendaftaran_igd/browse');
	}
	
	public function load_data($browse = 0) {
		
		$aColumns = array('tanggal', 'no_register', 'no_medrec', 'nama', 'jenis_kelamin', 'alamat_jalan', 'tanggal_lahir', 'agama', 'pendidikan', 'pekerjaan', 'rujukan', 'cara_bayar', 'dokter');
		...
		$pendaftaran = $this->Pendaftaran_IGD_Model->getAll($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
		...
		foreach ($rResult as $pendaftaran) {
			...
			$row[] = "<a href=\"".site_url("pendaftaran_igd/edit/".$pendaftaran->id)."\" data-original-title=\"Edit\" title=\"Edit Pendaftaran Rawat Jalan\" style=\"color: #0C9ABB\">".$tanggal."</a>&nbsp;";
			...
			switch ($browse) {
				case 0:
				case 1:
					$action = "<a class=\"btn btn-info btn-mini\" href=\"".site_url("pendaftaran_igd/edit/".$pendaftaran->id)."\" data-original-title=\"Edit\" title=\"Edit Pendaftaran Rawat Jalan\">Edit</a>&nbsp;";
	...
	public function delete() {
		if ($this->input->get('id')) {
			...
			$this->Pendaftaran_IGD_Model->delete($id);
		}
    }
	...
	public function batal() {
 		if ($this->input->get('id')) {
			$id = $this->input->get('id');
			$this->Pendaftaran_IGD_Model->batal($id);
		}
    }
	
	public function browse_pasien() {
		$this->load->view('pendaftaran_igd/browse_pasien');
	}
	...
	public function get_pendaftaran_by_id() {
		...
		$pendaftaran = $this->Pendaftaran_IGD_Model->getBy(array('pendaftaran_igd.id' => $pendaftaran_id));
	...
	public function print_form_001($id) {
		$this->data['data'] = $this->Pendaftaran_IGD_Model->getBy(array('pendaftaran_igd.id' => $id));
		$this->load->view('pendaftaran_igd/form_001', $this->data);
	}
	...
	private function _order_no_register() {
		...
		$register = $this->session->userdata('register_no_register_igd');
		$register_id = $this->session->userdata('register_no_register_igd_id');
		// Jika sudah register hapus kemudian pesan
		if ($register) {
			$this->Pendaftaran_IGD_Model->delete_no_register_from_queue($register_id);
		}
		// Jika belum langsung pesan
		$no_register = $this->Pendaftaran_IGD_Model->get_no_register();
		$this->session->set_userdata('register_no_register_igd', TRUE);
		$this->session->set_userdata('register_no_register_igd_id', $no_register['no_register_queue_id']);
	...
	private function _order_no_medrec() {
		...
		// Jika sudah register hapus kemudian pesan
		if ($register) {
			$this->Pendaftaran_IGD_Model->delete_no_medrec_from_queue($register_id);
		}
		// Jika belum langsung pesan
		$no_register = $this->Pendaftaran_IGD_Model->get_no_medrec();
	...
	public function load_data_report($laporan = "") {
		...
		*/
		...
		switch($laporan) {
			case "browse_001":
				$reports = $this->Pendaftaran_IGD_Model->get_kunjungan_per_klinik($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
				break;
			case "browse_002":
				$reports = $this->Pendaftaran_IGD_Model->get_kunjungan_per_cara_bayar($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
				break;
			case "browse_003":
				$reports = $this->Pendaftaran_IGD_Model->get_kunjungan_per_wilayah($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
				break;
			case "browse_004":
				$reports = $this->Pendaftaran_IGD_Model->get_kunjungan_per_cara_kunjungan($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
				break;
			case "browse_005":
				$reports = $this->Pendaftaran_IGD_Model->get_buku_pasien_register($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
				break;
			case "browse_006":
				$reports = $this->Pendaftaran_IGD_Model->get_kartu_rekam_medis($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
				break;
			case "browse_007":
				$reports = $this->Pendaftaran_IGD_Model->get_bank_nomor_rekam_medis($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
				break;
		}
	...
	public function to_print($rpt = "") {
		...
		switch($rpt) {
			case "browse_001":
				$data = $this->Pendaftaran_IGD_Model->get_kunjungan_per_klinik(0);
				break;
			case "browse_002":
				$data = $this->Pendaftaran_IGD_Model->get_kunjungan_per_cara_bayar(0);
				break;
			case "browse_003":
				$data = $this->Pendaftaran_IGD_Model->get_kunjungan_per_wilayah(0);
				break;
			case "browse_004":
				$data = $this->Pendaftaran_IGD_Model->get_kunjungan_per_cara_kunjungan(0);
				break;
			case "browse_005":
				$data = $this->Pendaftaran_IGD_Model->get_buku_pasien_register(0);
				break;
			case "browse_006":
				$data = $this->Pendaftaran_IGD_Model->get_kartu_rekam_medis(0);
				break;
			case "browse_007":
				$data = $this->Pendaftaran_IGD_Model->get_bank_nomor_rekam_medis(0);
				break;
		}
		...
		$this->load->view('pendaftaran_igd/'.$rpt, $this->data);
	}
...
/* End of file pendaftaran_igd.php */
/* Location: ./application/modules/pendaftaran_igd/pendaftaran_igd.php */