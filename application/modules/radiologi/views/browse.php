<script type="text/javascript">
	$(document).ready(function () {
		...
		var oTable = $('#pendaftaran').dataTable({
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php echo site_url('radiologi/pendaftaran/load_data/'.$browse); ?>",
			"sScrollX"			: "100%",
		...
		$('#pendaftaran').on('click', '.delete-row', function() {
			...
			alertify.confirm("Hapus record tersebut?", function (e) {
				if (e) {
					$.ajax({
						type: 'get',
						url: '<?php echo site_url('radiologi/pendaftaran/delete'); ?>',
						data: 'id=' + id,
						success: function() {
							oTable.fnDraw();
		...
		$('#pendaftaran').on('click', '.batal-row', function() {
			...
			alertify.confirm("Batalkan record tersebut?", function (e) {
				if (e) {
					$.ajax({
						type: 'get',
						url: '<?php echo site_url('radiologi/pendaftaran/batal'); ?>',
						data: 'id=' + id,
						success: function() {
							oTable.fnDraw();
