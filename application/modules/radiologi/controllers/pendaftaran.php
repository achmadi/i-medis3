...
class Pendaftaran extends Admin_Controller {
	...
	public function __construct() {
		...
		$this->load->model('radiologi/Pendaftaran_Rad_Model');
		$this->load->model('radiologi/Pendaftaran_Rad_Detail_Model');
		...
		$this->Pendaftaran_Rad_Model->delete_expire_no_register_from_queue();
	}
	...
	private function _update($id = 0) {
        ...
        if ($this->input->post('Batal')) {
            redirect('radiologi/pendaftaran', 'refresh');
        }
        ...
        if ($this->input->post('simpan')) {
            if ($this->form_validation->run() == TRUE) {
                ...
                if (isset($pendaftaran->pendaftaran_id) && $pendaftaran->pendaftaran_id > 0) {
                    $this->Pendaftaran_Rad_Model->update($pendaftaran);
                }
                else {
                    $this->Pendaftaran_Rad_Model->create($pendaftaran);
                }
				redirect('radiologi/pendaftaran/index?print='.$id, 'refresh');
            }
        }
		
		if ($id) {
			$this->data['new_pasien'] = false;
			$pendaftaran = $this->Pendaftaran_Rad_Model->getBy(array('id' => $id));
		...
		$this->data['pj_hubungan_list'] = $this->Pendaftaran_Rad_Model->getHubunganDenganPasien();
		
		$gedungs = $this->Gedung_Model->getAll(0);
		$this->data['gedung_list'] = $gedungs['data'];
		
		$this->data['top_nav'] = "radiologi/top_nav";
		$this->data['top_nav_selected'] = "Pendaftaran";
		$this->data['sub_nav'] = "radiologi/sub_nav";
		$this->template->set_title('Pendaftaran Rawat Jalan')
			           ...
			           ->build('radiologi/edit');
    }
	
	public function load_data_kegiatan_radiologi() {
		$unit = $this->Unit_Model->getBy(array('jenis' => $this->config->item('ID_RADIOLOGI')));
		if ($unit) {
	...
	public function browse($browse = 0) {
		...
		$this->template->set_title('Pendaftaran Radiologi')
					   ->set_css('alertify.core', array('id' => 'toggleCSS'))
					   ->set_js('jquery.dataTables')
					   ->set_js('FixedColumns.min')
					   ->set_js('alertify.min')
			           ->build('radiologi/browse');
	}
	...
	public function load_data($browse = 0) {
		...
		$pendaftaran = $this->Pendaftaran_Rad_Model->getAll($iLimit, $iOffset, $aOrders, $aWheres, $aLikes);
		...
		foreach ($rResult as $pendaftaran) {
			...
			switch ($browse) {
				case 0:
				case 1:
					$action = "<a class=\"btn btn-info btn-mini\" href=\"".site_url("radiologi/pendaftaran/edit/".$pendaftaran->id)."\" data-original-title=\"Edit\" title=\"Edit Pendaftaran Rawat Jalan\">Edit</a>&nbsp;";
	...
	public function delete() {
		if ($this->input->get('id')) {
			...
			$this->Pendaftaran_Rad_Model->delete($id);
		}
    }
	...
	public function batal() {
 		if ($this->input->get('id')) {
			$id = $this->input->get('id');
			$this->Pendaftaran_Rad_Model->batal($id);
		}
    }
	
	public function browse_pasien() {
		$this->load->view('radiologi/browse_pasien');
	}
	...
	public function doPrint($id) {
		$this->data['data'] = $this->Pendaftaran_Rad_Model->getBy(array('id' => $id));
		$this->load->view('radiologi/pendaftaran/print', $this->data);
	}
	...
	public function laporan() {
		$this->data['title'] = "Laporan Radiologi";
		$this->data['top_nav'] = "radiologi/top_nav";
		$this->data['top_nav_selected'] = "Laporan";
		$this->data['sub_nav'] = "radiologi/sub_nav3";
		$this->data['sub_nav_selected'] = "Laporan Radiologi";
		$this->template->set_title('Laporan Radiologi')
					   ->set_css('datepicker')
					   ->set_js('bootstrap-datepicker')
					   ->set_js('date-picker/date')
					   ->set_js('date-picker/daterangepicker')
					   ->set_js('jquery.dataTables')
			           ->build('radiologi/browse_001');
	}
    ...
    private function _getDataObject() {
		...
		$data = $this->Pendaftaran_Rad_Detail_Model->getAll(0, 0, array(), array(), array());
        $pendaftaran->pemeriksaans = $data['data'];
        
		$aPemeriksaans = array();
        foreach ($pendaftaran->pemeriksaans as $pemeriksaan) {
            if ($pemeriksaan->radiologi_id != $pendaftaran->pendaftaran_id) {
                $pemeriksaan->radiologi_id = $pendaftaran->pendaftaran_id;
            }
	...
	private function _order_no_register() {
		...
		$register = $this->session->userdata('register_no_register_rad');
		$register_id = $this->session->userdata('register_no_register_rad_id');
		// Jika sudah register hapus kemudian pesan
		if ($register) {
			$this->Pendaftaran_Rad_Model->delete_no_register_from_queue($register_id);
		}
		// Jika belum langsung pesan
		$no_register = $this->Pendaftaran_Rad_Model->get_no_register();
		$this->session->set_userdata('register_no_register_rad', TRUE);
		$this->session->set_userdata('register_no_register_rad_id', $no_register['no_register_queue_id']);
...
/* Location: ./application/modules/radiologi/pendaftaran.php */