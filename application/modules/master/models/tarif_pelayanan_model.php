...
class Tarif_Pelayanan_Model extends CI_Model {
	...
	protected $table_def = "tarif_pelayanan";
	...
	public function build_tree($parent_id = 0) {
		$results = array();
		...
		if ($query->num_rows() > 0) {
			...
			foreach($result as $index => $row) {
				$new_node = array();
				$new_node['key'] = $row->id;
				$new_node['title'] = $row->nama;
				$results[] = $new_node;
			}
			foreach($results as $index => $row) {
				$children = $this->build_tree($row['key']);
				if (!empty($children)) {
					$results[$index]['isFolder'] = true;
