<script type="text/javascript">
	$(document).ready(function () {
		...
		var oTable = $('#pasien').dataTable({
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php echo site_url('master/pasien/load_data'); ?>",
			"aoColumnDefs"		: [
		...
		$("#advanced-search").click(function() {
			//$.getJSON("<?php echo site_url('master/pasien/get_data'); ?>?pasien_id=0", function(json) {
		...
		$("#tambah-row").click(function() {
			$.getJSON("<?php echo site_url('master/pasien/get_data'); ?>?pasien_id=0", function(json) {
				$("#pasien_title_dlg").text("Tambah Pasien");
		...
		$('#pasien').on('click', '.delete-row', function() {
			var id = $(this).data('id');
			alertify.set({
				labels: {
					ok: "OK",
			...
			alertify.confirm("Hapus record tersebut?", function (e) {
				if (e) {
					$.ajax({
						type: 'get',
						url: '<?php echo site_url('master/pasien/delete'); ?>',
						data: 'id=' + id,
						success: function() {
							oTable.fnDraw();
		...
		$("#provinsi_id").change(function() {
			...
			$("#loading_kabupaten").show();
			$.getJSON("<?php echo site_url('master/pasien/get_data_kabupaten'); ?>?provinsi_id=" + provinsi_id, function(json) {
				var option_kabupaten = '';
				option_kabupaten += '<option value="0" selected="selected">- Pilih Kabupaten -</option>';
				if (json.success) {
					for (var i = 0; i < json.data.length; i++) {
						option_kabupaten += '<option value="' + json.data[i].id + '">' + json.data[i].nama + '</option>';
					}
					$("#kabupaten_id").html(option_kabupaten);
				}
				else {
		...
		$("#kabupaten_id").change(function() {
			...
			$("#loading_kecamatan").show();
			$.getJSON("<?php echo site_url('master/pasien/get_data_kecamatan'); ?>?kabupaten_id=" + kabupaten_id, function(json) {
				var option_kecamatan = '';
				option_kecamatan += '<option value="0" selected="selected">- Pilih Kecamatan -</option>';
				if (json.success) {
					for (var i = 0; i < json.data.length; i++) {
						option_kecamatan += '<option value="' + json.data[i].id + '">' + json.data[i].nama + '</option>';
					}
					$("#kecamatan_id").html(option_kecamatan);
				}
				else {
		...
		$("#kecamatan_id").change(function() {
			...
			$("#loading_kelurahan").show();
			$.getJSON("<?php echo site_url('master/pasien/get_data_kelurahan'); ?>?kecamatan_id=" + kecamatan_id, function(json) {
				var option_kelurahan = '';
				option_kelurahan += '<option value="0" selected="selected">- Pilih Kelurahan -</option>';
				if (json.success) {
					for (var i = 0; i < json.data.length; i++) {
						option_kelurahan += '<option value="' + json.data[i].id + '">' + json.data[i].nama + '</option>';
					}
					$("#kelurahan_id").html(option_kelurahan);
				}
				else {
...
<style type="text/css">
	...
	#advanced_search_modal .modal-body {
        max-height: 800px;
    }
		...
		<div class="span12">
			<div class="widget">
				...
				<div class="form-actions no-margin">
					<a id="advanced-search" class="btn btn-info bottom-margin pull-left" href="#" data-original-title="">Advanced Search</a>
					<a id="tambah-row" class="btn btn-info bottom-margin pull-right" href="#" data-original-title="">Tambah</a>
				...
				<div class="widget-body">
					<div id="dt_example" class="example_alt_pagination">
						<table class="table table-striped table-condensed table-striped table-hover table-bordered pull-left" id="pasien">
							...
							<tbody>
								<tr>
									<td colspan="24" class="dataTables_empty">Loading data from server</td>
								</tr>
							</tbody>
						</table>
						<div class="clearfix"></div>
...
<form class="form-horizontal no-margin" id="advanced_search_form" name="advanced_search_form" method="post" action="">
	<div class="modal hide fade" id="advanced_search_modal">
		<div class="modal-header">
			<a class="close" data-dismiss="modal">&times;</a>
			<h4 id="title_dlg">Advanced Search</h4>
		</div>
		<div class="modal-body">
			<div class="modal-body">
				<div class="row-fluid">
		...
		<div class="modal-footer">
			<button type="submit" id="cari_pasien" class="btn btn-primary">Cari</button>
			<button type="reset" class="btn" data-dismiss="modal">Batal</button>
		</div>
	</div>
</form>