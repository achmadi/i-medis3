<script type="text/javascript">
	$(document).ready(function() {
		...
		if (master_obat_id > 0) {
                    $.ajax({
                            type: "GET",
                            url: "<?php echo site_url('gudang/penerimaan/get_satuan') ?>",
		...
		on('changeDate', function(ev) {
			...
			var date_str = getISODate(date);
                        var entry = Date.parse(date_str);
                        var now = Date.parse(curDate);
                        if(entry <= now){
                            alert("Tanggal kadaluarsa harus lebih besar dari tanggal hari ini!");
                            $('#disp_exp_date').val(getFormDate(curDate));
                            $('#disp_exp_date').css('color','red');
                            $('#exp_date').val(getISODate(curDate));
                        } else {
                            $('#disp_exp_date').css('color','black');
    ...
    <div class="row-fluid">
        <div class="span12">
            <div class="widget">
                <?php
                    if ($is_new) {
                        $url = site_url('gudang/penerimaan/add');
                        $title = "Tambah Penerimaan";
                        $extraId = "master_obat_id";
                        $extraAttr = "";
                        $extraHid = "";
                    } else {
                        $url = site_url('gudang/penerimaan/edit/'.$data->gud_penerimaan_id);
                ...
                ?>
                <form class="form-horizontal no-margin" id="penerimaan_form" name="penerimaan_form" method="post" action="<?php echo $url; ?>">
