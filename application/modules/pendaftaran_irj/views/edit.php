<script type="text/javascript">
	...
	function print() {
		<?php
			if ($print_baru) {
		?>
			window.open("<?php echo site_url('pendaftaran_irj/print_form_001/'.$print_id); ?>", "Print Tracer", "scrollbars=1, height=300, width=300");
			window.open("<?php echo site_url('pendaftaran_irj/print_form_002/'.$print_id); ?>", "Pendaftaran Pasien", "scrollbars=1, height=300, width=300");
			window.open("<?php echo site_url('pendaftaran_irj/print_form_003/'.$print_id); ?>", "Print Kartu Status Front", "scrollbars=1, height=300, width=300");
			window.open("<?php echo site_url('pendaftaran_irj/print_form_004/'.$print_id); ?>", "Print Kartu Status Back", "scrollbars=1, height=300, width=300");
			window.open("<?php echo site_url('pendaftaran_irj/print_form_005/'.$print_id); ?>", "Print Kartu Anggota", "scrollbars=1, height=300, width=300");
		<?php
			}
			else {
		?>
		...
		if (old_id !== id) {
			$.getJSON("<?php echo site_url('pendaftaran_irj/get_pasien_by_id'); ?>" + "?pasien_id=" + id, function(json) {
				...
				if (kecamatan_id > 0) {
					$.ajax({
						type: "GET",
						url: "<?php echo site_url('pendaftaran_irj/get_kelurahan') ?>",
	...
	$(document).ready(function() {
	   ...
	   function getISODate(d){
		...
		if (poliklinik_id > 0) {
			$.ajax({
				type: "GET",
				url: "<?php echo site_url('pendaftaran_irj/get_dokter') ?>",
		...
		var oTable = $('#pasien').dataTable({
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?php echo site_url('pendaftaran_irj/pendaftaran_irj/load_data_pasien'); ?>",
			"aoColumns"			: [
									  { sWidth: '5%' },
									  { sWidth: 'null' },
									  { sWidth: '20%' },
		...
		$("#generate_medrec").click(function() {
			$.ajax({
				url			: "<?php echo site_url('pendaftaran_irj/generate_medrec') ?>",
				dataType	: "json",
				cache		: false,
				beforeSend: function() {
		...
		$("#no_medrec").keypress(function(event) {
			var is_generate = $("#is_generate_medrec").val();
			if (!parseInt(is_generate)) {
				if (event.which === 13) {
					event.preventDefault();
					var no_medrec = $("#no_medrec").val();
					$.getJSON("<?php echo site_url('pendaftaran_irj/get_pasien_by_no_medrec'); ?>" + "/" + no_medrec, function(json) {
						if (json.status === "ok") {
							$("#pasien_id").val(json.pasien.id);
							$("#no_medrec").val(json.pasien.no_medrec);
							$("#nama").val(json.pasien.nama);
	...
	<div class="row-fluid">
		<div class="span12">
			<div class="widget">
				<?php
					if ($is_new) {
						$url = site_url('pendaftaran_irj/pendaftaran_irj');
					}
					else {
						$url = site_url('pendaftaran_irj/pendaftaran_irj/edit/'.$data->pendaftaran_id.'?print');
					}
				?>
				<form class="form-horizontal no-margin" id="pendaftaran_form" name="pendaftaran_form" method="post" action="<?php echo $url; ?>">
								...
								<div class="span6">
									...
									<div class="control-group">
										...
										<div class="controls controls-row">
											...
											?>
											<input class="span12" type="text" id="nama" name="nama" placeholder="Nama" value="<?php echo $value; ?>" autocomplete="off" />
								...
								<div class="span6">
									...
									<div class="control-group">
										...
										<div class="controls controls-row">
											...
											?>
											<select id="poliklinik_id" name="poliklinik_id" class="span12">
												<?php
											...
											<?php echo form_error('poliklinik_id'); ?>
										</div>
									</div>
									...
									<div class="control-group">
										...
										<div class="controls controls-row">
											...
											<img id="loading_dokter" alt="loading" src="<?php echo base_url('assets/img/loading.gif'); ?>" style="position: absolute; margin: 5px; display: none;" />
										</div>
									</div>
					...
					<div>
						...
						<input type="hidden" id="is_generate_medrec" value="0" />
					</div>
				</form>
			</div>
		</div>
...
<div class="modal fade" id="pasien_modal" data-remote="<?php echo site_url('pendaftaran_irj/pendaftaran_irj/browse_pasien'); ?>" >
